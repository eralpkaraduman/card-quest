...
But there is usually couple other ways to do implement routing;
...
Project started as a regular react-native app generated by the template;

`npx react-native init card-quest --template react-native-template-typescript` 

React native project comes with it's own bundler called [Metro](https://facebook.github.io/metro/). 
...
`index.web.ts` is where webpack looks first for bundling the web app. And `index.native.ts` is where react-native app starts to build the mobile app bundle. You can see the project structure in above segment: "Option 3: Install react-dom into react-native project". In summary both bundlers exclude the other platform's specific files. Webpack config excludes `*.native.ts` files and Metro config excludes `*.web.ts`. Webpack config also has an alias from react-native modules to react-native-web modules;
...
Respectively in metro configuration, we exclude web specific files. Metro intrinsicly knows to look at `index.android.js` and `index.ios.js` first so entry point to `index.native.ts` is configured there ;
...
This part covers how the project is put together and what kind of choices I took among other options. There are other topics which are more like implementation details. So I'm planning to cover these topics in separate articles in the future, some of these are already implemented you can check how they are built also in [the source code](card-quest-source);

### Linking:

About how to handle platform specific page routes, tab bars and navigation stacks are managed. This is briefly covered in above "Platform specific code" section.

### Dialogs:

When app shows dialogs, web app displays the same content in a [reach ui dialog](https://reach.tech/dialog/), but the native app displays it in [react-native-bottom-sheet](https://gorhom.github.io/react-native-bottom-sheet/). I will be going into detail about their usage.
...
This project has been just an experiment and hasn't been tested on a real-life consumer facing application, so beware that most of what I have been talking about are not best practices. However all these choices ended up working very nicely so I'm hopeful that they can work well on a production app also. If you choose to take a similar path for implementing cross platform apps, please let me know! You can reach me from [eralp.dev].
